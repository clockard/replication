...
public class ResourceImpl implements Resource {
  ...
  public ResourceImpl(ReplicationMetadata metadata) {
    this(
        metadata.getId(),
        new ByteSource() {
          @Override
          public InputStream openStream() {
            ...
            try {
              if (metadata.getResourceUri() == null) {
                throw new ReplicationException(
                    "Cannot get resource. No resource for " + metadata.getId());
              }
              return new URL(metadata.getResourceUri().toASCIIString()).openStream();
            } catch (IOException e) {
              throw new ReplicationException(
                  "Could not open stream for " + metadata.getResourceUri());
            }
          }
        },
   ...
   */
  public ResourceImpl(
      String id, ByteSource byteSource, String mimeTypeRawData, ReplicationMetadata metacard) {
  ...
  public InputStream getInputStream() {
    ...
    try {
      ...
      throw new ReplicationException("Could not open stream for " + id);
    }
  }
  ...
  public String toString() {
    return String.format(
        ...
        id, uri, filename, mimeTypeRawData, mimeType, byteSource, size, metadata, qualifier);
  }
}
